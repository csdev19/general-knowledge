...
_Functional error handling with the Result type pattern._
...
const result = await tryCatch(
  db.select().from(entities).where(eq(entities.id, entityId))
);
```
...
```typescript
import { isFailure, isSuccess } from "@your-scope/domain/types";
...
if (isFailure(result)) {
  throw new InternalServerError("Failed to fetch entity");
}
...
```typescript
const result = await tryCatch(fetchEntityFromAPI(entityId));
...
3. **Use with error handlers**: Combine with [Error Handlers](./error-handlers.md) for consistent error handling.

4. **Type your errors**: Specify error types when possible:
   ```typescript
   type Result<Entity, DatabaseError>
   ```

## See Also

- [Error Handlers](./error-handlers.md) — Utilities for handling Result types
- [Response Helpers](./response-helpers.md) — Creating API responses
