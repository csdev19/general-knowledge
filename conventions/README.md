...
- **[ci-cd-pipeline-strategy.md](./ci-cd-pipeline-strategy.md)** — Trunk + tags, one `verify` gate, tiered checks; the native/mobile install is the biggest CI cost (split it out); `deps:weight` protocol to watch dependency weight. Normative rules now in [monorepos/release-gated-verification.md](../monorepos/release-gated-verification.md).
