...
| [infra/](./infra/) | Dominios, DNS y edge routing: **migración de un dominio a Cloudflare Workers** — los registros de parking heredados que rompen el deploy, apex vs `www`, y el checklist de todo lo que invalida un origin nuevo (auth, OAuth, CORS, deep links); **secretos con Infisical** — una sola fuente de valores que los scripts traen al arrancar, sin `.env` mantenidos a mano, OIDC en CI; y la evaluación de varlock como capa de declaración (y por qué no es el default) |
