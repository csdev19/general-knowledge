...
| [dotfiles/](./dotfiles/) | Bootstrap de workstation reproducible: por qué `brew bundle` (y equivalentes) no sirven para un bootstrap resumible, `set -e` matando el script entero por un solo paquete, paths de config auto-localizados (nunca hardcodear dónde se clona el repo), alias SSH vs. nombres de archivo de llave sobreviviendo un restore, e instaladores de terceros que escriben directo en tus dotfiles versionados |
