...
_Architecture for the Hono + oRPC API pattern with a TanStack Start web app. Contract-first, REST-style URLs, OpenAPI-compatible._
...
- **Cross-app contract import**: Web app imports contract from the server via relative path (same pattern as Eden Treaty's `type { App }` import)
- **Domain schemas reused**: `todoBaseSchema`, `createTodoSchema`, etc. from `@app/domain/schemas`
...
│   └── utils/
│       ├── errors.ts               # Custom error classes (framework-agnostic)
│       └── response-helpers.ts     # successBody, errorBody helpers
├── package.json                    # name: "server-hono"
├── tsconfig.json
└── wrangler.jsonc                  # name: "<app>-api"
```
...
│   │   ├── api-fetch.ts            # Service Binding fetch wrapper
│   │   ├── auth/
...
│   ├── server-functions/
│   │   └── list-todos.ts           # TanStack serverFn for paginated list
│   ├── routes/
...
├── tsconfig.json
├── vite.config.ts
└── wrangler.jsonc                  # name: "<app>-web", service: "<app>-api"
```
...
import { todoBaseSchema, createTodoSchema, updateTodoSchema, paginationQuerySchema } from "@app/domain/schemas";
...
Uses the standalone `os` builder to define context-aware middleware:
...
```json
{
  ...
  "@app/infra-auth": "workspace:*",
  "@app/infra-db": "workspace:*",
  "@app/infra-env": "workspace:*"
}
```
...
- **Server wrangler name**: `<app>-api`
- **Web wrangler name**: `<app>-web`
- **Service Binding**: `API_SERVICE` → `<app>-api`
...
See [API Contract Pattern](./api-contract-pattern.md) for steps to move the contract from `apps/server-hono/src/contract/` into a dedicated `packages/api-contract/` workspace package.
